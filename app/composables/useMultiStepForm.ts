...
> {
  ...
  onError?: (error: any, ctx: MultiStepFormContext<T, D>) => Promise<void> | void
}
 ...
 */
export function createMultiStepForm<T extends Record<string, any>, D extends Record<string, any>>(
  ...
  const currentStep = computed(() => {
    const step = steps.value.find((step) => step.to === router.currentRoute.value.path)?.id
  ...
  const errors = reactive({ name: "", avatar: "" })
...
const multiStepFormContextInjectionKey = Symbol("multi-step-form-context") as InjectionKey<MultiStepFormContext>

export function useMultiStepForm<T extends Record<string, any>, D extends Record<string, any>>() {
  const context = inject(multiStepFormContextInjectionKey as InjectionKey<MultiStepFormContext<T, D>>)
  if (!context) {
    throw new Error("MultiStepForm: no context found, did you forget to call createMultiStepForm?")
  }
